...
tmpfs paths, anonymous `/data`, and read-only project bind, but container removal discards that
anonymous state. Clients that ignore declared OCI runtime arguments are unsupported. The verified
Compose/launcher bundle remains the maintained durability and operator-control surface.
